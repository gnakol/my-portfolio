...
export class CandidacyService {
  ...
  async create(createCandidacyDto: CreateCandidacyDto): Promise<CandidacyResponseDto> {
    try {
      ...
      // Création automatique des 3 reminders (J+3, J+7, J+14)
      await this.createAutomaticReminders(savedCandidacy.id, savedCandidacy.applicationDate);
  ...
  async update(
    ...
    try {
      ...
      // Détecter si la date de candidature a changé pour mettre à jour les reminders
      const applicationDateChanged =
        updateCandidacyDto.applicationDate &&
        new Date(updateCandidacyDto.applicationDate).getTime() !==
          new Date(candidacy.applicationDate).getTime();

      Object.assign(candidacy, updateCandidacyDto);
      const updatedCandidacy = await this.candidacyRepository.save(candidacy);

      // Si la date de candidature a changé, recalculer les reminders automatiques
      if (applicationDateChanged) {
        await this.updateAutomaticReminders(
          updatedCandidacy.id,
          updatedCandidacy.applicationDate,
        );
      }
  ...
  /**
   * Créer automatiquement 3 reminders (J+5, J+10, J+15) pour une candidature
   */
  private async createAutomaticReminders(
    ...
    try {
      ...
      const reminderDays = [5, 10, 15]; // Mise à jour: J+5, J+10, J+15
      const reminderTypes = [
        ReminderType.FIRST_FOLLOW_UP,
        ReminderType.SECOND_FOLLOW_UP,
        ReminderType.FINAL_FOLLOW_UP,
      ];

      for (let i = 0; i < reminderDays.length; i++) {
        const dueDate = new Date(baseDate);
        dueDate.setDate(baseDate.getDate() + reminderDays[i]);
        ...
        await this.reminderService.create({
          ...
          comment: `Relance automatique J+${reminderDays[i]}`,
          autoGenerated: true,
        });
      }

      this.logger.log(`3 automatic reminders created for candidacy ID: ${candidacyId}`);
    } catch (error) {
      this.logger.warn(
        `Failed to create automatic reminders for candidacy ${candidacyId}: ${error.message}`,
      );
      // Ne pas bloquer la création de la candidature si les reminders échouent
    }
  }

  /**
   * Mettre à jour les dates des reminders automatiques quand la date de candidature change
   */
  private async updateAutomaticReminders(
    candidacyId: number,
    newApplicationDate: Date,
  ): Promise<void> {
    try {
      // Récupérer tous les reminders auto-générés pour cette candidature
      ...
      if (reminders.length === 0) {
        this.logger.log(
          `No auto-generated reminders found for candidacy ${candidacyId}, nothing to update`,
        );
        return;
      }

      const baseDate = new Date(newApplicationDate);
      const reminderDays = [5, 10, 15]; // J+5, J+10, J+15

      // Mettre à jour chaque reminder avec la nouvelle date calculée
      for (let i = 0; i < reminders.length && i < reminderDays.length; i++) {
        const reminder = reminders[i];
        const newDueDate = new Date(baseDate);
        newDueDate.setDate(baseDate.getDate() + reminderDays[i]);

        reminder.dueDate = newDueDate;
        reminder.comment = `Relance automatique J+${reminderDays[i]}`;

        await this.reminderRepository.save(reminder);
        this.logger.log(
          `Updated reminder ${reminder.id} to new due date: ${newDueDate.toISOString()}`,
        );
      }

      this.logger.log(
        `Updated ${reminders.length} automatic reminders for candidacy ${candidacyId}`,
      );
    } catch (error) {
      this.logger.warn(
        `Failed to update automatic reminders for candidacy ${candidacyId}: ${error.message}`,
      );
      // Ne pas bloquer la mise à jour de la candidature si les reminders échouent
    }
  }
    ...
    return {
      ...
      jobOffer: candidacy.jobOffer
        ? {
            ...
            offerUrl: candidacy.jobOffer.offerUrl,
            platform: candidacy.jobOffer.platform,
            locationCity: candidacy.jobOffer.locationCity,
            locationCountry: candidacy.jobOffer.locationCountry,
            minSalary: candidacy.jobOffer.minSalary,
            maxSalary: candidacy.jobOffer.maxSalary,
            typeOfContract: candidacy.jobOffer.typeOfContract,
            experienceLevel: candidacy.jobOffer.experienceLevel,
            remoteMode: candidacy.jobOffer.remoteMode,
            publicationDate: candidacy.jobOffer.publicationDate,
            expirationDate: candidacy.jobOffer.expirationDate,
            createdAt: candidacy.jobOffer.createdAt,
          }
        : undefined,
    };
  }
